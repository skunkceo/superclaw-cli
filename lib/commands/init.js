...
async function initWorkspace() {
  ...
  try {
    ...
    console.log('1. Clawdbot (cloud-based, easy setup)');
    console.log('2. OpenClaw (self-hosted, more control)');
    console.log('3. Other (manual configuration)\n');
    
    const backendChoice = await ask('Choose [1-3]: ');
    let backend = 'other';
    ...
    if (backend === 'clawdbot') {
      console.log(`\n${colors.dim}For Clawdbot setup, you'll need your API key.${colors.reset}`);
    } else if (backend === 'openclaw') {
      console.log(`\n${colors.dim}For OpenClaw, make sure your instance is running first.${colors.reset}`);
    }
