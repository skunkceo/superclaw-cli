...
async function installDashboard() {
  ...
  try {
    ...
    try {
      ...
      const tempDir = path.join(installDir, '.temp-clone');
      
      // Clone to temp directory
      execSync(
        `git clone https://github.com/skunkceo/superclaw-dashboard "${tempDir}"`,
        { stdio: 'pipe' }
      );
      
      // Move contents from temp to install dir
    ...
    if (!setupNow.toLowerCase().startsWith('n')) {
      ...
      if (!startNow.toLowerCase().startsWith('n')) {
        ...
        // Start dashboard in background with output to file
