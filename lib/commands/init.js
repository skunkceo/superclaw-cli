...
async function installDashboard() {
  ...
  try {
    ...
    info('Installing packages (this takes 2-4 minutes on first run)...\n');

    // Spinner so the terminal doesn't look frozen
    const spinnerFrames = ['⠋','⠙','⠹','⠸','⠼','⠴','⠦','⠧','⠇','⠏'];
    let spinnerIdx = 0;
    const spinnerInterval = setInterval(() => {
      process.stdout.write(`\r${colors.dim}${spinnerFrames[spinnerIdx % spinnerFrames.length]} Installing...${colors.reset}`);
      spinnerIdx++;
    }, 100);
    ...
    try {
      ...
      // Retry without --prefer-offline in case of cache miss
      info('Retrying with full download...');
      try {
        ...
        console.log(`\n${colors.dim}Try manually: cd ${installDir} && npm install${colors.reset}\n`);
        process.exit(1);
      }
    }
    ...
    info('Building (this also takes a minute)...\n');

    let buildSpinnerIdx = 0;
    const buildSpinner = setInterval(() => {
      process.stdout.write(`\r${colors.dim}${spinnerFrames[buildSpinnerIdx % spinnerFrames.length]} Building...${colors.reset}`);
      buildSpinnerIdx++;
    }, 100);
