...
async function installDashboard() {
  ...
  try {
    ...
    await checkDashboardUpdate(installDir);
...
/**
 * Check for dashboard updates after installation
 */
async function checkDashboardUpdate(installDir) {
  try {
    // Read local version
    const versionPath = path.join(installDir, 'version.json');
    if (!fs.existsSync(versionPath)) {
      return; // No version.json, skip check
    }
    ...
    if (latestVersion && latestVersion !== localVersion) {
      console.log(`\n${colors.yellow}! Update available: v${localVersion} → v${latestVersion}${colors.reset}`);
      console.log(`  Run: ${colors.cyan}superclaw update${colors.reset}\n`);
    }
  } catch (e) {
    // Silently fail - don't interrupt installation success
  }
}
