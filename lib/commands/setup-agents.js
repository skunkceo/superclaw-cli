...
async function run(args) {
  ...
  try {
    ...
    // Update .env file for SuperClaw dashboard (if it exists)
    const possibleDashboardPaths = [
      path.join(process.cwd(), 'superclaw-dashboard'),
      path.join(homeDir, 'superclaw-dashboard'),
      path.join(homeDir, 'apps', 'websites', 'superclaw-dashboard'),
      '/home/mike/apps/websites/superclaw-dashboard'
    ];
    
    for (const dashboardPath of possibleDashboardPaths) {
      if (fs.existsSync(dashboardPath)) {
        ...
        success(`Updated dashboard .env: ${envPath}`);
        break;
      }
    }
    ...
    console.log(`  • OpenClaw workspace: ${openclawWorkspace}\n`);
