 ...
 * - Product Manager
 */
async function setupAgents(rl, openclawDetected = false) {
  ...
  try {
    ...
    if (!openclawWorkspace) {
      throw new Error('Workspace not found in OpenClaw status');
    }
  } catch (e) {
    ...
    if (!homeDir) {
      ...
      console.log(`  1. Install OpenClaw: ${colors.cyan}npm install -g openclaw${colors.reset}`);
      console.log(`  2. Set HOME environment variable`);
      console.log(`  3. Skip agent setup for now\n`);
      
      const skip = await ask('Skip agent setup? [y/N]: ');
      if (skip.toLowerCase().startsWith('y')) {
        warn('Skipping agent setup. You can configure agents later via the dashboard.');
        return;
      } else {
        throw new Error('Cannot proceed without workspace directory');
      }
    }
    
    openclawWorkspace = path.join(homeDir, '.openclaw', 'workspace');
    
    // Only show warning if OpenClaw wasn't detected earlier in the flow
    if (!openclawDetected) {
      warn(`OpenClaw not detected. Using default workspace: ${openclawWorkspace}`);
    } else {
      info(`Using OpenClaw workspace: ${openclawWorkspace}`);
    }
  }
  
  // Verify workspace path is valid
  if (!openclawWorkspace || typeof openclawWorkspace !== 'string') {
  ...
  try {
    // Resolve templates directory - works for both local and global npm installs
  ...
  const routingRulesTemplate = path.resolve(__dirname, '..', 'templates', 'routing-rules.json');
...
`;
    ...
    console.log(`\n${colors.dim}Details: ${err.stack}${colors.reset}\n`);
    console.log(`${colors.yellow}Agent setup failed, but you can configure agents later via the dashboard.${colors.reset}\n`);
